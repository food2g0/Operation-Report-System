...
class AdminDashboard(QWidget):
    ...
    def load_report_corporations(self):
        """Load corporations for report dialog (batch with OS list)"""
        try:
            # Use batch queries to load both in one round-trip
            results = db_manager.execute_batch([
                {
                    "sql": "SELECT name as corporation FROM corporations ORDER BY name",
                    "params": None,
                    "ttl": 300  # Cache for 5 minutes (rarely changes)
                },
                {
                    "sql": "SELECT DISTINCT os_name FROM branches WHERE os_name IS NOT NULL AND os_name != '' ORDER BY os_name",
                    "params": None,
                    "ttl": 300
                }
            ])
            ...
            if results and len(results) >= 2:
                # Load corporations
                corps_result = results[0].get("result", [])
                if corps_result:
                    for row in corps_result:
                        corp_name = row['corporation'] if isinstance(row, dict) else row[0]
                        self.report_corp_combo.addItem(corp_name)

                # Load OS list (while we're at it)
                os_result = results[1].get("result", [])
                if os_result:
                    for row in os_result:
                        os_name = row['os_name'] if isinstance(row, dict) else row[0]
                        self.report_os_combo.addItem(os_name)
        except Exception as e:
            logger.error("Error loading corporations and OS list: %s", e)

    def load_report_os_list(self):
        """Load OS list for report dialog (called by load_report_corporations via batch)"""
        # Already loaded by load_report_corporations, so nothing to do here
        pass
    ...
    def generate_daily_cash_report(self, dialog):
        ...
        by_corp = self.report_by_corp_radio.isChecked()
        filter_value = self.report_corp_combo.currentText() if by_corp else self.report_os_combo.currentText()
        filter_label = "Corporation" if by_corp else "Group"
        ...
        # BATCH OPTIMIZATION: Fetch columns and report data in ONE round-trip
        batch_queries = [
            {
                "sql": "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = %s",
                "params": [self.daily_table],
                "ttl": 3600  # Cache for 1 hour (schema doesn't change often)
            }
        ]

        # Add main report query to batch
        if by_corp:
            batch_queries.append({
                "sql": f"""
                    SELECT branch, beginning_balance, {', '.join(debit_columns)},
                           debit_total, {', '.join(credit_columns)},
                ...
                "ttl": 0  # Don't cache report data
            })
        else:
            batch_queries.append({
                "sql": f"""
                    SELECT dr.branch, dr.beginning_balance, {', '.join(['dr.' + c for c in debit_columns])},
                           dr.debit_total, {', '.join(['dr.' + c for c in credit_columns])},
                           dr.credit_total, dr.ending_balance, dr.cash_count, dr.cash_result
        ...
        # Execute batch query (1 HTTP call instead of 2+)
        batch_results = db_manager.execute_batch(batch_queries)

        if not batch_results or len(batch_results) < 2:
            progress.close()
            QMessageBox.warning(self, "Error", "Failed to load report data.")
            return

        # Extract column info
        col_rows = batch_results[0].get("result", [])
        ...
        results = batch_results[1].get("result")
