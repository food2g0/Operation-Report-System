...
class ClientDashboard(QWidget):
    ...
    def __init__(self, username, branch, corporation, db_manager, offline_mode=False):
        ...
        self.balance_manager = BalanceManager(db_manager, branch, corporation, offline_mode, username)
        self.palawan_manager = PalawanManager(db_manager, branch, corporation)
        ...
        try:
            ...
            for brand_full, data in brand_data.items():
                ...
                if result is not None:
                    results.append(brand_full)
                    # Only save Brand B's Palawan Details B data to payable_tbl_brand_a
    ...
    def get_previous_day_ending_balance(self, selected_date, brand="Brand A"):
        """Get ending balance from previous day (delegates to BalanceManager)."""
        return self.balance_manager.get_previous_day_ending_balance(selected_date, brand)

    def _get_offline_previous_balance(self, selected_date, brand):
        """Get offline previous balance (delegates to BalanceManager)."""
        return self.balance_manager._get_offline_previous_balance(selected_date, brand)
    ...
    def _get_palawan_adjustments(self, date_str):
        """Fetch palawan adjustments (delegates to PalawanManager)."""
        return self.palawan_manager.get_palawan_adjustments(date_str)
    ...
    def _get_brand_specific_palawan_adjustments(self, date_str, brand_full):
        return self.palawan_manager.get_brand_specific_palawan_adjustments(date_str, brand_full)

    def _save_palawan_to_payable(self, date_str, brand_full, palawan_data, brand_adjustments=None):
        return self.palawan_manager.save_palawan_to_payable(date_str, brand_full, palawan_data, brand_adjustments)
    ...
    def handle_post(self):
        ...
        try:
            ...
            for brand_full, cf_tab, bb_input, cc_input, table_name in brand_specs:
                ...
                balances = self.post_handler.calculate_balances(cf, beginning, cash_count)
                ending = balances['ending_balance']
                cash_result = balances['cash_result']
                variance_status = balances['variance_status']
                deb = balances['debit_total'] - beginning
                cre = balances['credit_total']

                all_vals = self.post_handler.merge_cash_flow_values(cf)

                bank_account = getattr(cf_tab, 'selected_bank_account', None)
                ft_ho_breakdown = (self._ft_ho_breakdowns.get(brand_full)
                                  if hasattr(self, '_ft_ho_breakdowns') else None)
                pc_salary = self._pc_salary_breakdown if hasattr(self, '_pc_salary_breakdown') else None
                motor_car = self._motor_car_breakdown if hasattr(self, '_motor_car_breakdown') else None

                all_vals = self.post_handler.collect_optional_fields(
                    all_vals, cf_tab, brand_full,
                    bank_account=bank_account,
                    ft_ho_breakdown=ft_ho_breakdown,
                ...
                if not is_valid:
                    results.append((brand_full, "validation_error", error_msg))
                    continue
                ...
                if isinstance(rows, int) and rows > 0:
                    ...
                    if not is_valid:
                        ...
                        # CRITICAL: Only Brand B's Palawan Details B tab data saves to payable_tbl_brand_a
    ...
    def _connect_palawan_adjustments_to_brand_b(self):
        self.palawan_manager.connect_palawan_adjustments_to_brand_b(
            self.palawan_tab, self.cash_flow_tab_b, self.recalculate_all
        )
    ...
    def _collect_palawan_for_draft(self):
        return self.palawan_manager.collect_palawan_for_draft(self.palawan_tab)
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            is_valid, error_msg = self.export_handler.validate_file_path(file_path)
            if not is_valid:
            ...
            self.export_handler.build_workbook(
