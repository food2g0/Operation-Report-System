...
from admin.manage import (create_corporation, create_branch, create_client,
...
class UserManagementPage(QWidget):
    ...
    def __init__(self, is_super_admin=False):
        ...
        self._ensure_rescuardo_column()
        self.setup_ui()

    def _ensure_rescuardo_column(self):
        try:
            self.db.execute_query(
                "ALTER TABLE branches ADD COLUMN rescuardo VARCHAR(3) NULL DEFAULT NULL"
            )
        except Exception:
    ...
    def _build_branches_tab(self):
        ...
        self.branch_rescuardo_combo = QComboBox()
        self.branch_rescuardo_combo.addItem("-- Rescuardo --", None)
        self.branch_rescuardo_combo.addItem("YES", "YES")
        self.branch_rescuardo_combo.addItem("NO", "NO")
        self.branch_rescuardo_combo.setMinimumWidth(110)
        ...
        row2_layout.addWidget(QLabel("Rescuardo:"))
        row2_layout.addWidget(self.branch_rescuardo_combo)
        ...
        self.branch_table.setColumnCount(14)
        self.branch_table.setHorizontalHeaderLabels(["ID", "Branch Name", "Corporation", "Sub Corporation", "OS", "Area", "Global", "Sunday", "LOB", "Payroll", "Rescuardo", "Created At", "Registered", "Actions"])
        ...
        # Col 1 (Branch Name) stretches; all others are Interactive so the user can resize
        col_widths = {
            0: 45,   # ID
            1: None, # Branch Name — Stretch
            2: 160,  # Corporation
            3: 140,  # Sub Corporation
            4: 130,  # OS / Group
            5: 100,  # Area
            6: 90,   # Global
            7: 75,   # Sunday
        ...
        for col, width in col_widths.items():
            if width is None:
                ...
                self.branch_table.setColumnWidth(col, width)
        ...
        try:
            ...
            rows = self.db.execute_query(f"""
                SELECT b.id, b.name, c.name as corp_name, b.created_at, b.is_registered,
                       ...
                       COALESCE(b.rescuardo, '') as rescuardo,
            ...
            if rows:
                for row_data in rows:
                    ...
                    # Rescuardo column (col 10)
                    rescuardo_val = row_data.get('rescuardo', '') or ''
                    rescuardo_widget = QWidget()
                    rescuardo_lay = QHBoxLayout(rescuardo_widget)
                    rescuardo_lay.setContentsMargins(2, 2, 2, 2)
                    rescuardo_lay.setSpacing(4)
                    rescuardo_label = QLabel(rescuardo_val if rescuardo_val else '—')
                    if rescuardo_val.upper() == 'YES':
                        rescuardo_label.setStyleSheet("font-size:10px; color: #8e44ad; font-weight: bold;")
                    else:
                        rescuardo_label.setStyleSheet("font-size:10px; color: #aaa;")
                    rescuardo_edit_btn = QPushButton("✏️")
                    rescuardo_edit_btn.setFixedSize(24, 24)
                    rescuardo_edit_btn.setToolTip("Edit Rescuardo Tag")
                    rescuardo_edit_btn.setStyleSheet("QPushButton { background: #3498db; border: none; border-radius: 3px; } QPushButton:hover { background: #2980b9; }")
                    rescuardo_edit_btn.clicked.connect(lambda checked, bid=row_data['id'], bname=row_data.get('name', ''), cur=rescuardo_val: self._edit_branch_dropdown(bid, bname, 'rescuardo', 'Rescuardo', cur, ['YES', 'NO']))
                    rescuardo_lay.addWidget(rescuardo_label, 1)
                    rescuardo_lay.addWidget(rescuardo_edit_btn)
                    self.branch_table.setCellWidget(row_idx, 10, rescuardo_widget)

                    self.branch_table.setItem(row_idx, 11, QTableWidgetItem(str(row_data.get('created_at', ''))[:19]))
                    ...
                    self.branch_table.setCellWidget(row_idx, 12, reg_widget)
                    ...
                    action_layout.addWidget(del_btn)
                    self.branch_table.setCellWidget(row_idx, 13, action_widget)
    ...
    def _add_branch(self):
        ...
        rescuardo = self.branch_rescuardo_combo.currentData()
        ...
        try:
            ...
            if bid:
                ...
                self.db.execute_query(
                    """UPDATE branches
                       SET area = %s, global_tag = %s, sunday = %s, line_of_business = %s, rescuardo = %s
                       WHERE id = %s""",
                    (area, global_tag, sunday, lob, rescuardo, bid)
                )
            ...
            self.branch_rescuardo_combo.setCurrentIndex(0)
