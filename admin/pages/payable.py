...
from admin.date_range_widget import DateRangeWidget
import logging
...
class PayablesPage(QWidget):
    ...
    def populate_table(self):
        ...
        try:
            # Fallback approach: Get palawan data from payable_tbl_brand_a (new) or daily_reports (old)
            ...
            if is_range:
                lotes_join = (
                    ...
                    "SUM(COALESCE(sendout_lotes, 0)) AS so_lotes, "
                    "SUM(COALESCE(sendout_capital, 0)) AS so_capital, "
                    "SUM(COALESCE(sendout_sc, 0)) AS so_sc, "
                    "SUM(COALESCE(sendout_commission, 0)) AS so_commission, "
                    "SUM(COALESCE(sendout_total, 0)) AS so_total, "
                    "SUM(COALESCE(payout_lotes, 0)) AS po_lotes, "
                    "SUM(COALESCE(payout_capital, 0)) AS po_capital, "
                    "SUM(COALESCE(payout_sc, 0)) AS po_sc, "
                    "SUM(COALESCE(payout_commission, 0)) AS po_commission, "
                    "SUM(COALESCE(payout_total, 0)) AS po_total, "
                    "SUM(COALESCE(international_lotes, 0)) AS int_lotes, "
                    "SUM(COALESCE(international_capital, 0)) AS int_capital, "
                    "SUM(COALESCE(international_sc, 0)) AS int_sc, "
                    "SUM(COALESCE(international_commission, 0)) AS int_commission, "
                    "SUM(COALESCE(international_total, 0)) AS int_total, "
                    "SUM(COALESCE(skid, 0)) AS pt_skid, "
                    "SUM(COALESCE(skir, 0)) AS pt_skir, "
                    "SUM(COALESCE(cancellation, 0)) AS pt_cancellation, "
                    "SUM(COALESCE(inc, 0)) AS pt_inc "
                    f"FROM payable_tbl_brand_a WHERE date BETWEEN %s AND %s GROUP BY corporation, branch "
                    f"UNION "
                    f"SELECT corporation, branch, "
                    f"SUM(COALESCE(palawan_sendout_lotes_total, 0)) AS so_lotes, "
                    f"SUM(COALESCE(palawan_sendout_principal, 0)) AS so_capital, "
                    f"SUM(COALESCE(palawan_sendout_sc, 0)) AS so_sc, "
                    f"SUM(COALESCE(palawan_sendout_commission, 0)) AS so_commission, "
                    f"SUM(COALESCE(palawan_sendout_regular_total, 0)) AS so_total, "
                    f"SUM(COALESCE(palawan_payout_lotes_total, 0)) AS po_lotes, "
                    f"SUM(COALESCE(palawan_payout_principal, 0)) AS po_capital, "
                    f"SUM(COALESCE(palawan_payout_sc, 0)) AS po_sc, "
                    f"SUM(COALESCE(palawan_payout_commission, 0)) AS po_commission, "
                    f"SUM(COALESCE(palawan_payout_regular_total, 0)) AS po_total, "
                    f"SUM(COALESCE(palawan_international_lotes_total, 0)) AS int_lotes, "
                    f"SUM(COALESCE(palawan_international_principal, 0)) AS int_capital, "
                    f"SUM(COALESCE(palawan_international_sc, 0)) AS int_sc, "
                    f"SUM(COALESCE(palawan_international_commission, 0)) AS int_commission, "
                    f"SUM(COALESCE(palawan_international_regular_total, 0)) AS int_total, "
                    f"SUM(COALESCE(palawan_suki_discounts, 0)) AS pt_skid, "
                    f"SUM(COALESCE(palawan_suki_rebates, 0)) AS pt_skir, "
                    f"SUM(COALESCE(palawan_cancel, 0)) AS pt_cancellation, "
                    f"SUM(COALESCE(palawan_pay_out_incentives, 0)) AS pt_inc "
                    f"FROM {self.daily_table} WHERE date BETWEEN %s AND %s "
                    f"AND NOT EXISTS (SELECT 1 FROM payable_tbl_brand_a p WHERE p.branch COLLATE utf8mb4_general_ci = {self.daily_table}.branch COLLATE utf8mb4_general_ci AND p.corporation COLLATE utf8mb4_general_ci = {self.daily_table}.corporation COLLATE utf8mb4_general_ci AND p.date = {self.daily_table}.date) "
                    f"GROUP BY corporation, branch) pt "
                ...
                lotes_params = [date_start, date_end, date_start, date_end]
            else:
                lotes_join = (
                    "LEFT JOIN (SELECT corporation, branch, "
                    "MAX(COALESCE(sendout_lotes, 0)) AS so_lotes, "
                    "MAX(COALESCE(sendout_capital, 0)) AS so_capital, "
                    "MAX(COALESCE(sendout_sc, 0)) AS so_sc, "
                    "MAX(COALESCE(sendout_commission, 0)) AS so_commission, "
                    "MAX(COALESCE(sendout_total, 0)) AS so_total, "
                    "MAX(COALESCE(payout_lotes, 0)) AS po_lotes, "
                    "MAX(COALESCE(payout_capital, 0)) AS po_capital, "
                    "MAX(COALESCE(payout_sc, 0)) AS po_sc, "
                    "MAX(COALESCE(payout_commission, 0)) AS po_commission, "
                    "MAX(COALESCE(payout_total, 0)) AS po_total, "
                    "MAX(COALESCE(international_lotes, 0)) AS int_lotes, "
                    "MAX(COALESCE(international_capital, 0)) AS int_capital, "
                    "MAX(COALESCE(international_sc, 0)) AS int_sc, "
                    "MAX(COALESCE(international_commission, 0)) AS int_commission, "
                    "MAX(COALESCE(international_total, 0)) AS int_total, "
                    "MAX(COALESCE(skid, 0)) AS pt_skid, "
                    "MAX(COALESCE(skir, 0)) AS pt_skir, "
                    "MAX(COALESCE(cancellation, 0)) AS pt_cancellation, "
                    "MAX(COALESCE(inc, 0)) AS pt_inc "
                    f"FROM payable_tbl_brand_a WHERE date = %s GROUP BY corporation, branch "
                    f"UNION "
                    f"SELECT corporation, branch, "
                    f"MAX(COALESCE(palawan_sendout_lotes_total, 0)) AS so_lotes, "
                    f"MAX(COALESCE(palawan_sendout_principal, 0)) AS so_capital, "
                    f"MAX(COALESCE(palawan_sendout_sc, 0)) AS so_sc, "
                    f"MAX(COALESCE(palawan_sendout_commission, 0)) AS so_commission, "
                    f"MAX(COALESCE(palawan_sendout_regular_total, 0)) AS so_total, "
                    f"MAX(COALESCE(palawan_payout_lotes_total, 0)) AS po_lotes, "
                    f"MAX(COALESCE(palawan_payout_principal, 0)) AS po_capital, "
                    f"MAX(COALESCE(palawan_payout_sc, 0)) AS po_sc, "
                    f"MAX(COALESCE(palawan_payout_commission, 0)) AS po_commission, "
                    f"MAX(COALESCE(palawan_payout_regular_total, 0)) AS po_total, "
                    f"MAX(COALESCE(palawan_international_lotes_total, 0)) AS int_lotes, "
                    f"MAX(COALESCE(palawan_international_principal, 0)) AS int_capital, "
                    f"MAX(COALESCE(palawan_international_sc, 0)) AS int_sc, "
                    f"MAX(COALESCE(palawan_international_commission, 0)) AS int_commission, "
                    f"MAX(COALESCE(palawan_international_regular_total, 0)) AS int_total, "
                    f"MAX(COALESCE(palawan_suki_discounts, 0)) AS pt_skid, "
                    f"MAX(COALESCE(palawan_suki_rebates, 0)) AS pt_skir, "
                    f"MAX(COALESCE(palawan_cancel, 0)) AS pt_cancellation, "
                    f"MAX(COALESCE(palawan_pay_out_incentives, 0)) AS pt_inc "
                    f"FROM {self.daily_table} WHERE date = %s "
                    f"AND NOT EXISTS (SELECT 1 FROM payable_tbl_brand_a p WHERE p.branch COLLATE utf8mb4_general_ci = {self.daily_table}.branch COLLATE utf8mb4_general_ci AND p.corporation COLLATE utf8mb4_general_ci = {self.daily_table}.corporation COLLATE utf8mb4_general_ci AND p.date = {self.daily_table}.date) "
                    f"GROUP BY corporation, branch) pt "
                    "ON b.name COLLATE utf8mb4_general_ci = pt.branch COLLATE utf8mb4_general_ci "
                    "AND COALESCE(dr.corporation, c.name) COLLATE utf8mb4_general_ci = pt.corporation COLLATE utf8mb4_general_ci"
                )
                lotes_params = [date_start, date_start]
