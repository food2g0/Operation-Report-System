...
class ReportPage(QWidget):
    ...
    def get_totals_from_database(self):
        ...
        try:
            # Determine date clause (outer query uses src.date)
            is_range = self.date_range_widget.is_range_mode()
            if is_range:
                date_clause = "src.date >= %s AND src.date <= %s"
                date_params = (date_start, date_end)
            else:
                date_clause = "src.date = %s"
                date_params = (date_start,)

            # Build corp filter clause using src. alias (from the UNION subquery).
            # Global Reliance uses global_tag via branches join. For all other corporations
            # we match where EITHER the stored corporation name matches OR the branch
            # belongs to the selected corporation (as main or sub-corporation).
            if is_global_reliance:
                ...
                corp_clause = """(
                    src.corporation = %s
                    OR src.branch COLLATE utf8mb4_general_ci IN (
            ...
            # Palawan totals query: payable_tbl_brand_a is the primary source.
            # A UNION ALL fallback covers rows that exist only in daily_reports
            # (not yet uploaded via the palawan client form).
            # This ensures palawan adjustments are included even when a branch has
            # a payable_tbl_brand_a row for that date but no daily_reports anchor.
            _src = f"""(
                SELECT p.corporation, p.branch, p.date,
                       COALESCE(p.sendout_capital, 0)         AS sc,
                       COALESCE(p.sendout_commission, 0)      AS scom,
                       COALESCE(p.sendout_sc, 0)              AS ssc,
                       COALESCE(p.payout_capital, 0)          AS pc,
                       COALESCE(p.payout_commission, 0)       AS pcom,
                       COALESCE(p.payout_sc, 0)               AS psc,
                       COALESCE(p.international_commission, 0) AS icom,
                       COALESCE(p.skid, 0)                    AS skid,
                       COALESCE(p.skir, 0)                    AS skir,
                       COALESCE(p.cancellation, 0)            AS cancellation,
                       COALESCE(p.inc, 0)                     AS inc
                FROM payable_tbl_brand_a p

                UNION ALL

                SELECT dr.corporation, dr.branch, dr.date,
                       COALESCE(dr.palawan_sendout_principal, 0),
                       COALESCE(dr.palawan_sendout_commission, 0),
                       COALESCE(dr.palawan_sendout_sc, 0),
                       COALESCE(dr.palawan_payout_principal, 0),
                       COALESCE(dr.palawan_payout_commission, 0),
                       COALESCE(dr.palawan_payout_sc, 0),
                       COALESCE(dr.palawan_international_commission, 0),
                       COALESCE(dr.palawan_suki_discounts, 0),
                       COALESCE(dr.palawan_suki_rebates, 0),
                       COALESCE(dr.palawan_cancel, 0),
                       COALESCE(dr.palawan_pay_out_incentives, 0)
                FROM {self.daily_table} dr
                WHERE NOT EXISTS (
                    SELECT 1 FROM payable_tbl_brand_a p2
                    WHERE p2.corporation COLLATE utf8mb4_general_ci = dr.corporation COLLATE utf8mb4_general_ci
                      AND p2.branch COLLATE utf8mb4_general_ci = dr.branch COLLATE utf8mb4_general_ci
                      AND p2.date = dr.date
                )
            ) src"""

            _pal_select = """
                SUM(src.sc)           AS total_sendout_capital,
                SUM(src.scom)         AS total_sendout_commission,
                SUM(src.ssc)          AS total_sendout_sc,
                SUM(src.pc)           AS total_payout_capital,
                SUM(src.pcom)         AS total_payout_commission,
                SUM(src.psc)          AS total_payout_sc,
                SUM(src.icom)         AS total_international_commission,
                SUM(src.skid)         AS total_skid,
                SUM(src.skir)         AS total_skir,
                SUM(src.cancellation) AS total_cancellation,
                SUM(src.inc)          AS total_inc"""

            result = db_manager.execute_query(f"""
                SELECT {_pal_select}
                FROM {_src}
                LEFT JOIN branches b
                  ON src.branch COLLATE utf8mb4_general_ci = b.name COLLATE utf8mb4_general_ci
                WHERE {corp_clause}
                  AND {date_clause}
                  {reg_clause}
