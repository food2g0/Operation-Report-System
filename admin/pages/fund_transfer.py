...
from admin.db_worker import run_func_async
...
class FundTransferPage(QWidget):
    def __init__(self, account_type=2):
        ...
        self.rescuardo_filter_label = QLabel("Rescuardo:")
        self.rescuardo_filter_selector = QComboBox()
        self.rescuardo_filter_selector.addItem("All Branches", "all")
        self.rescuardo_filter_selector.addItem("Rescuardo Only", "yes")
        ...
        filter_row.addWidget(self.rescuardo_filter_label)
        filter_row.addWidget(self.rescuardo_filter_selector)
        ...
        if account_type == 2:  # Brand B: filter by corporation
            ...
            self.os_label.setVisible(False)
            self.os_selector.setVisible(False)
            self.rescuardo_filter_label.setVisible(True)
            self.rescuardo_filter_selector.setVisible(True)
            ...
            self.rescuardo_filter_label.setVisible(False)
            self.rescuardo_filter_selector.setVisible(False)
    ...
    def populate_table(self):
        ...
        # Build registration clause for the inner subquery (no alias)
        inner_reg_clause = ""
        if reg_filter == "registered":
            inner_reg_clause = "AND is_registered = 1"
        elif reg_filter == "not_registered":
            inner_reg_clause = "AND (is_registered = 0 OR is_registered IS NULL)"

        # Build rescuardo clause (Brand B only)
        rescuardo_filter = self.rescuardo_filter_selector.currentData() if self.account_type == 2 else "all"
        rescuardo_clause = "AND rescuardo = 'YES'" if rescuardo_filter == "yes" else ""


        if report_type == "corporation":
            filter_value = self.corp_selector.currentText()
            is_rescuardo_only = (rescuardo_filter == "yes")
            if not filter_value and not is_rescuardo_only:
                return
            is_global_reliance = (not is_rescuardo_only) and ('GLOBAL RELIANCE' in filter_value.upper())

            # When rescuardo-only: show all branches with rescuardo='YES', ignore corporation filter
            if is_rescuardo_only:
                branch_where = "rescuardo = 'YES'"
                rescuardo_clause = ""  # already covered by branch_where
            elif is_global_reliance:
                branch_where = "global_tag = 'GLOBAL'"
            else:
                branch_where = (
            ...
            # Params: rescuardo-only and global_reliance need no corporation params
            no_corp_params = is_rescuardo_only or is_global_reliance
            ...
            if is_range:
                ...
                """
                params = (date_start, date_end) if no_corp_params else (filter_value, filter_value, date_start, date_end)
            else:
                query = f"""
                    SELECT b.name as branch,
                           COALESCE(b.area, 'UNASSIGNED') as area,
                    ...
                    ORDER BY COALESCE(b.area, 'ZZZZZ'), b.name
                """
                params = (date_start,) if no_corp_params else (filter_value, filter_value, date_start)
        else:  # OS mode
